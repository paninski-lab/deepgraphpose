...
def get_optimizer(loss_op, cfg):
    ...
    # if cfg.train_prediction_net:
    variables_to_train = all_variables_to_train
    # else:
    # variables_to_train = list(filter(lambda k: 'pose' in k.name, all_variables_to_train))
...
def train(config_yaml,displayiters,saveiters,maxiters,max_to_keep=5,keepdeconvweights=True,allow_growth=False):
    ...
    for it in range(max_iter):
        ...
        if it % display_iters == 0 and it>0:
            ...
            cum_loss = 0.0
            logging.info("iteration: {} loss: {} lr: {}, running time:{}"
                         .format(it, "{0:.4f}".format(average_loss), current_lr, end_time00 - start_time00))
            lrf.write("{}, {:.5f}, {}, running time:{}\n".format(it, average_loss, current_lr, end_time00 - start_time00))
            lrf.flush()

        # Save snapshot
        if (it % save_iters == 0 and it != 0) or (it == max_iter-1):
