...
def create_new_project(project, experimenter, videos, working_directory=None, copy_videos=True,videotype='.avi',
                       relative_path=True, date=None,):
    ...
    if date is None:
        ...
        from datetime import datetime as dt
        datename = dt.strptime(date, '%Y-%m-%d')
        d = str(datename.strftime("%B")[0:3] + str(datename.day))
...
#                videos=[videos]
    ...
    for video in videos:
        ...
        # enforce relative video path
        # and read from copied video
#        if relative_path and copy_videos:
#            rel_video_path = str(os.path.join('videos', Path(videos[0]).name))
            # rel_video_path = rel_video_path

        vcap = cv2.VideoCapture(os.path.join(project_path, rel_video_path))

        if vcap.isOpened():
           width = int(vcap.get(cv2.CAP_PROP_FRAME_WIDTH))
           height = int(vcap.get(cv2.CAP_PROP_FRAME_HEIGHT))
           print('rel_video_path',rel_video_path)
