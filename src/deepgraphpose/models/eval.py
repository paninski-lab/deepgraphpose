...
def setup_dgp_eval_graph(dlc_cfg, dgp_model_file, loc_ref=False, gauss_len=1, gamma=1):
    ...
    with tf.variable_scope('pose', reuse=None):
        scmap = dgp_prediction_layer(
            None, None, dlc_cfg, net, name='part_pred', num_outputs=dlc_cfg.num_joints,
            init_flag=False, nc=None, train_flag=True, stride=dlc_cfg.deconvolutionstride)
        if loc_ref:
            locref = dgp_prediction_layer(
                None, None, dlc_cfg, net, name='locref_pred',
                num_outputs=dlc_cfg.num_joints * 2, init_flag=False, nc=None, train_flag=True,
...
def estimate_pose(
    ...
    if save_pose:
        ...
        export_pose_like_dlc(
            labels, os.path.basename(dgp_model_file), dlc_cfg.all_joints_names, save_file)
...
def evaluate_dgp(proj_cfg_file, dgp_model_file, shuffle=1, loc_ref=None, loc_ref_calc='dlc'):
    ...
    Data = pd.read_hdf(os.path.join(
            proj_config["project_path"], str(trainingsetfolder),
    ...
    datafn, metadatafn = auxiliaryfunctions.GetDataandMetaDataFilenames(
            trainingsetfolder, proj_config['TrainingFraction'][0], shuffle, proj_config)
    ...
    for imageindex, imagename in tqdm(enumerate(Data.index)):
        ...
        if loc_ref:
            ...
            if loc_ref_calc.lower() == 'dlc':
                outputs_np = sess.run(
                    [scmap_tf, locref_tf], feed_dict={inputs: image[None, :, :, :]})
                ...
                [lr, st] = sess.run(
                    [locref_tf, softmax_tensor], feed_dict={inputs: image[None, :, :, :]})
                ...
                nx = shape[0]
                ny = shape[1]
                xg, yg = np.meshgrid(np.linspace(0, nx - 1, nx), np.linspace(0, ny - 1, ny))
                ...
                for joint_idx in range(dlc_cfg.num_joints):
                    ...
                    pose_f8 = alpha.astype('float') * dlc_cfg.stride + 0.5 * dlc_cfg.stride + lr_j
                    spatial_soft_argmax = np.sum(
                        np.sum(st_j * alpha, 1), 1) * dlc_cfg.stride + 0.5 * dlc_cfg.stride
    ...
    RMSE, _ = pairwisedistances(
            DataCombined, proj_config["scorer"], DLCscorer, proj_config["pcutoff"],
            comparisonbodyparts)
...
def plot_dgp(
    ...
    if not os.path.exists(label_file):
        estimate_pose(
            proj_cfg_file, dgp_model_file, video_file, label_dir, shuffle=shuffle,
            save_str=save_str)
